...
export async function initializePaymentController(req, res) {
  try {
    const { firstName, lastName, email, costOfKey, numberOfKeys, phoneNumber } = req.body;
   ...
   return res.send({ success: true, message:"transaction created", url: authorization_url }).status(200);
  } catch (error) {
    console.error(error);
  }
}
